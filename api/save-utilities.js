...
const TARGET_PATH = 'data/utilities.json';
const DEFAULT_BRANCH = 'main';

/**
 * 管理画面から受け取ったデータを GitHub の utilities.json に保存する
 */
module.exports = async (req, res) => {
  ...
  try {
    ...
    const normalized = normalizeUtilityData(data);
    ...
    console.error('save-utilities error:', error);
...
function validateUtilityData(data) {
  ...
  if (!Array.isArray(data.utilities)) {
    throw createHttpError(400, 'utilities は配列である必要があります');
  }
}
